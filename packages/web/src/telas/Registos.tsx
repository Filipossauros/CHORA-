...
import { Estado, formatarDuracao, hoje, mensagemErro, useAsync } from '../comum.js';
...
export function Registos(): ReactNode {
  ...
  // Por omissão: hoje e 8 horas (480 min) de trabalho (feedback R5).
  const [form, setForm] = useState({ afetacaoId: '', data: hoje(), duracao: 480, descricao: '', workItemId: 1001, tipo: '' });
  ...
  async function registar(submeter: boolean): Promise<void> {
    setErro(undefined);
    try {
      const criado = await app.registos.criar({ afetacaoId: form.afetacaoId, workItemId: form.workItemId, data: form.data, duracao: form.duracao, descricaoAtividade: form.descricao, ...(form.tipo !== '' ? { tipoDotacaoConsumida: form.tipo as never } : {}) }, app.utilizador());
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 16 }}>
        <div className="cartao"><h3>Registar tempo</h3><div className="corpo">
          <div className="g2">
            ...
            <div className="campo"><label>Duração (min, múltiplos de 15)</label><input type="number" min={15} step={15} value={form.duracao} onChange={(e) => setForm({ ...form, duracao: Number(e.target.value) })} /></div>
          </div>
          <div className="campo"><label>Afetação (perfil · projeto)</label><select value={form.afetacaoId} onChange={(e) => setForm({ ...form, afetacaoId: e.target.value })}><option value="">— selecionar —</option>{afetacoes.map((a) => <option key={a.id} value={a.id}>{perfis.find((p) => p.id === a.perfilId)?.nome} · {a.projetoIds[0]}</option>)}</select></div>
        ...
        <div className="cartao"><h3>Registos</h3><table>
          ...
          <tbody>{registos.slice(0, 40).map((r) => <tr key={r.id}><td className="tabnum">{r.data}</td>{podeVerTodos && <td>{r.recursoId}</td>}<td className="num">{formatarDuracao(r.duracao)}</td><td>{r.descricaoAtividade}</td><td><Estado v={r.estado} /></td></tr>)}
