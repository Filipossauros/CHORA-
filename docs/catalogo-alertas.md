...
| AL-LICENCA-A-EXPIRAR | Licenciamento a expirar | A vigência das licenças aproxima-se do fim. Sem renovação, o direito de uso cessa na data indicada. | AVISO | fim da vigência do licenciamento | RN-113 | O licenciamento é um direito de uso por período determinado. | Renovar ou substituir o licenciamento não dispensa: a verificação de que a necessidade se mantém; a escolha do procedimento adequado ao valor e ao objeto, sem fracionamento da despesa; e o cabimento e compromisso prévios. A continuação de uso sem título válido não se regulariza a posteriori. |
